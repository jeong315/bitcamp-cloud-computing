...
CREATE TABLE P1_APPL (
    ...
    STAT INTEGER NULL     DEFAULT 0 COMMENT '신청 상태' -- 신청 상태
)
COMMENT '수강 신청';

-- 수강 신청
ALTER TABLE P1_APPL
    ADD CONSTRAINT PK_P1_APPL -- 수강 신청 기본키
        PRIMARY KEY (
            ANO -- 수강신청번호
        );
...
CREATE TABLE P1_STUD (
    ...
    LST-SCHL VARCHAR(50)  NOT NULL COMMENT '최종학력', -- 최종학력
...
CREATE TABLE P1_CLS_ROOM (
    ...
    MAX_QTY INTEGER     NOT NULL COMMENT '최대수용인원' -- 최대수용인원
)
COMMENT '강의실';
...
CREATE TABLE P1_OPEN_CORS (
    OCNO   INTEGER     NOT NULL COMMENT '개설교육과정번호', -- 개설교육과정번호
    CNO    VARCHAR(50) NOT NULL COMMENT '교육기수', -- 교육기수
    ...
    COL    INTEGER     NOT NULL COMMENT '교육과정번호', -- 교육과정번호
...
ALTER TABLE P1_OPEN_CORS
    ADD CONSTRAINT PK_P1_OPEN_CORS -- 개설 교육 과정 기본키
        PRIMARY KEY (
            OCNO -- 개설교육과정번호
        );
...
-- 과목
CREATE TABLE P1_SUB (
    SNO  INTEGER      NOT NULL COMMENT '교육과목번호', -- 교육과목번호
    TITL VARCHAR(255) NOT NULL COMMENT '과목명', -- 과목명
    CONT VARCHAR(255) NOT NULL COMMENT '과목설명' -- 과목설명
)
COMMENT '과목';
...
ALTER TABLE P1_SUB
    MODIFY COLUMN SNO INTEGER NOT NULL AUTO_INCREMENT COMMENT '교육과목번호';
...
ALTER TABLE P1_CLR_PHOTO
    ADD CONSTRAINT PK_P1_CLR_PHOTO -- 강의실 사진 기본키
        PRIMARY KEY (
            CPNO -- 강의실사진번호
        );
...
CREATE TABLE P1_CORS (
    COL3 INTEGER            NOT NULL COMMENT '교육과정번호', -- 교육과정번호
    COL  VARCHAR(255)       NOT NULL COMMENT '과정명', -- 과정명
    COL2 <데이터 타입 없음> NOT NULL COMMENT '과정설명' -- 과정설명
)
COMMENT '교육과정';
...
ALTER TABLE P1_CORS
    MODIFY COLUMN COL3 INTEGER NOT NULL AUTO_INCREMENT COMMENT '교육과정번호';
...
-- 수강 신청
ALTER TABLE P1_APPL
    ADD CONSTRAINT FK_P1_STUD_TO_P1_APPL -- 학생 -> 수강 신청
        FOREIGN KEY (
            SNO -- 학생번호
        )
...
-- 수강 신청
ALTER TABLE P1_APPL
    ADD CONSTRAINT FK_P1_OPEN_CORS_TO_P1_APPL -- 개설 교육 과정 -> 수강 신청
        FOREIGN KEY (
            OCNO -- 개설교육과정번호
        )
        REFERENCES P1_OPEN_CORS ( -- 개설 교육 과정
            OCNO -- 개설교육과정번호
        );
...
-- 개설 교육 과정
ALTER TABLE P1_OPEN_CORS
    ADD CONSTRAINT FK_P1_CLS_ROOM_TO_P1_OPEN_CORS -- 강의실 -> 개설 교육 과정
        FOREIGN KEY (
            CLRNO -- 강의실번호
        )
...
-- 개설 교육 과정
ALTER TABLE P1_OPEN_CORS
    ADD CONSTRAINT FK_P1_MGR_TO_P1_OPEN_CORS -- 매니저 -> 개설 교육 과정
        FOREIGN KEY (
            MGNO -- 매니저번호
        )
...
-- 개설 교육 과정
ALTER TABLE P1_OPEN_CORS
    ADD CONSTRAINT FK_P1_CORS_TO_P1_OPEN_CORS -- 교육과정 -> 개설 교육 과정
        FOREIGN KEY (
            COL -- 교육과정번호
        )
...
-- 프로젝트
ALTER TABLE P1_PRJ
    ADD CONSTRAINT FK_P1_OPEN_CORS_TO_P1_PRJ -- 개설 교육 과정 -> 프로젝트
        FOREIGN KEY (
            OCNO -- 개설교육과정번호
        )
        REFERENCES P1_OPEN_CORS ( -- 개설 교육 과정
...
-- 교육과정과목
ALTER TABLE P1_CORS_SUB
    ADD CONSTRAINT FK_P1_OPEN_CORS_TO_P1_CORS_SUB -- 개설 교육 과정 -> 교육과정과목
        FOREIGN KEY (
            OCNO -- 개설교육과정번호
        )
        REFERENCES P1_OPEN_CORS ( -- 개설 교육 과정
...
-- 교육과정 수강생
ALTER TABLE P1_CORS_STUD
    ADD CONSTRAINT FK_P1_OPEN_CORS_TO_P1_CORS_STUD -- 개설 교육 과정 -> 교육과정 수강생
        FOREIGN KEY (
            OCNO -- 개설교육과정번호
        )
        REFERENCES P1_OPEN_CORS ( -- 개설 교육 과정
...
-- 강의실 사진
ALTER TABLE P1_CLR_PHOTO
    ADD CONSTRAINT FK_P1_CLS_ROOM_TO_P1_CLR_PHOTO -- 강의실 -> 강의실 사진
        FOREIGN KEY (
            CLRNO -- 강의실번호
        )
        REFERENCES P1_CLS_ROOM ( -- 강의실
