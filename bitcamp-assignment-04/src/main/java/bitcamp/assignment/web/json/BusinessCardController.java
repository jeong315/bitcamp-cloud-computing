...
//    @Autowired
    ...
    public BusinessCardController(BusinessCardService bizcardService) {
        //생산자에서 생산자를 받는다. => 이런 방법도 있지만 Autowired가 더 편하다.
    ...
    @GetMapping("{no}")
    public Object get(@PathVariable int no, HttpSession session) {//@PathVariable Spring-WebMVC-src08
        Member loginUser = (Member)session.getAttribute("loginUser");
    ...
    @PostMapping("update")
    public Object update(BusinessCard bizcard, HttpSession session) {
