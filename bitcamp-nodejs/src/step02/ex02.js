...
var con = mysql.createConnection({
    host: '52.79.189.185', //호스트가 localhost면 생략가능
...
con.connect(function(err){
    //만약 연결에 실패했다면, err 파리미터 값이 존재할 것이다.
...
//    예약 작업이 실행될 것이다.
//    즉 연결 해체하라고 예약한 작업이 실행될 것이다.
// => end(DBMS와 연결을 끊은 후 호출될 함수)
con.end(function(err){// 지금 당장 연결을 끊으라는 것이 아니다.
    if(err) throw err;//만약 연결을 끊다가 오류가 있다면 예외를 던진다.
    console.log('연결을 끊었습니다.');//3
}); 
console.log('연결 테스트~!~!');//1
