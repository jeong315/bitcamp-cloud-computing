...
// => 문제는 연결 오류에 상관없이 무조건 예약 한다는 것이다.
...
con.query('select * from pms2_member', function(err, results){
    ...
    console.log("결과를 가져왔습니다.");//3
    for(var row of results){
        //row객체에서 값을 꺼낼 때는 SQL에서 지정한 컬럼명 또는 별명을 사용한다.
        console.log(row.email, row.mid, row.pwd);//4
    };
});
